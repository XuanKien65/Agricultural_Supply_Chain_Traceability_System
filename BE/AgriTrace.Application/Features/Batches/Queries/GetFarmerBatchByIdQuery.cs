    ...
    : IRequestHandler<GetFarmerBatchByIdQuery, FarmerBatchDto>
{
    public async Task<FarmerBatchDto> Handle(GetFarmerBatchByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (batch is null || (batch.CurrentOrganizationId.HasValue && batch.CurrentOrganizationId.Value != request.OrganizationId))
            throw new NotFoundException("Batch not found.");

        var product = await products.GetByIdAsync(batch.ProductId, cancellationToken)
            ?? new Domain.Entities.Product(batch.ProductId, "Sản phẩm nông sản", null, "kg", batch.CurrentOrganizationId ?? 0);
