...
export function FarmerDashboardPage() {
  ...
  const { data: page, isLoading } = useFarmerBatches(orgId)
  const data = page?.items ?? []
  return <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
    <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap' }}><Box><Typography color="primary" sx={{ fontWeight: 800 }}>CỔNG THÔNG TIN NÔNG HỘ</Typography><Typography variant="h4" sx={{ fontWeight: 900 }}>Xin chào, {user?.name}</Typography><Typography color="text.secondary">{user?.organizationName} • Theo dõi hoạt động sản xuất hôm nay</Typography></Box><Button component={Link} to="/batches/new" variant="contained" startIcon={<AgricultureRounded />}>Tạo lô thu hoạch</Button></Box>
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', lg: 'repeat(4,1fr)' }, gap: 2 }}>
      ...
      <StatCard title="Mới tạo" value={data.filter(x => x.status === 'ACTIVE').length} description="Đang tại nông trại" icon={<AgricultureRounded />} iconBg="#E3F2FD" iconColor="#1565C0" />
